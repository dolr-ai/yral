...
3. Next, you will be given the following things to start with:
   1. 3 servers to deploy a redundant load balancer. They also need to double up as a reverse proxy and a TLS termination point
...
4. Once done with setup, your first task is to deploy a simple "Hello World" service that returns "Hello World" on the `hello-world.<your_first_name>.yral.com` subdomain. The backing repo for this service and any other testing/dummy/experiment should reside on your personal organization account. Use the dolr-ai org to host only repositories that are for production services that are intended for yral's end users. The challenge with this task if for you to figure out redundancy and policies so that even if 1 of the servers is turned off or goes down, your service is still up and running and can serve traffic.
5. Once you have this up and running, you will change the text for the `hello-world` service to return "Hello visitor. You are the <visitor_count>'th visitor to this page". The visitor count needs to be stored in a redundant database that your application will connect to. It needs to increment the count for every visit made to this endpoint and return the incremented count in the response. This is to get you familiar with connecting to databases and managing state in a redundant way across multiple servers. You can choose any database you like.
6. When done with this, you are ready to tackle larger problems. We can assign more challenging and interesting problems for you to solve during the next morning meeting.
